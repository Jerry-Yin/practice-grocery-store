package com.greentech.ansibledt.goddesspad.view;
...
import com.greentech.ansibledt.goddesspad.R;
...
public class ArcProgress extends View {
    ...
    private int progressText = 0;  //中间文字
    private int max;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            //原来的进度文字，显示的是实时进度，会跟随进度条实时变化，跟进度绑定太死，
//            canvas.drawText(text, (getWidth() - textPaint.measureText(text)) / 2.0f, textBaseline, textPaint);

            //我自己定义的进度文字,没有跟进度绑定在一起，可以通过setProgressText()方法随时设置，需要外部手动设置
            String progText = String.valueOf(getProgressText());
            if (!TextUtils.isEmpty(progText)){
                Log.d(TAG, "curX = "+ (getWidth() - textPaint.measureText(progText)) / 2.0f);
                Log.d(TAG, "getWidth() = "+ getWidth());
                Log.d(TAG, "textWidth = "+ textPaint.measureText(text));
                canvas.drawText(progText, (float) (getWidth() - textPaint.measureText(progText)) / 2.0f, textBaseline, textPaint);
            }
